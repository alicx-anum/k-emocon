...
def parse_args():
    ...
    parser.add_argument('--lr', type=float, help='learning_rate', default=0.00005)
...
def main(args):
    ...
    # 读取CSV后，X是二维数组，行是样本，列是特征（时间点）
    # 只需要 unsqueeze(1) 让它变成 [batch, channel=1, length]
    X_train = torch.tensor(X_train, dtype=torch.float32).to(device).unsqueeze(1)  # [N, 1, length]
...
#==========打印形状===========
    ...
    #专为PHemo
    model = model_dict[args.model](dropout_rate=0.5, units=256, n=4, in_features=feature_len, num_classes=2).to(device)
