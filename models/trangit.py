...
'''
总体思路：自监督预训练——数据变换3种类型
每个模态的模型结构：CNN+Transformer编码
'''
# 设备选择
...
# Route configuration
# 数据集路径
# base_path：数据集根目录（相对路径示例）。
# class_data_path：拼接后指向存放分班/分类可穿戴数据的目录。
...
def load_and_preprocess_data(class_id, participant_id, window_size=240):  # 60 seconds at 4 Hz
    ...
    # 通过取步长为 16 的子采样来粗略将 BVP 重新采样到 4 Hz
    # 这样和eda、temp相同
    # （假设 BVP 原采样为 64 Hz；64/16=4）
    bvp = bvp.iloc[::16]
...
#============保证对齐================
    ...
    # 找出三通道都非 NaN 的行，按列合并成 shape (N_valid, 3) 的数组（列顺序为 EDA, BVP, TEMP）
    ...
    # Normalization z-score 标准化
    scaler = StandardScaler()
    data = scaler.fit_transform(data)

    # Divide into fixed-size windows 滑动窗口
    windows = []
...
#=========key1===========
# 自监督变换：对单个窗口做三种“变换”——原始、加噪、幅度扭曲，从而用于自监督预训练
...
# Transformer正弦位置编码实现
'''
初始 pe：(max_len, d_model)。
unsqueeze(0).transpose(0,1) 变为 (max_len, 1, d_model)，方便与输入 (seq_len, batch_size, d_model) 广播相加。
register_buffer('pe', pe)：将 pe 存为 module 的非参数缓冲（不会被优化，但会随 model.to(device) 移动）。
forward：期望输入 x 为 (seq_len, batch_size, d_model)（注意：此处与通常传入模型的维度顺序有关），把位置编码加上并做 dropout
'''
...
#========key3===========
# 1D卷积时间卷积网络（TCN）结构:两层 Conv1d + BN + ReLU + dropout
'''
padding=(kernel_size-1)//2 用以在 kernel_size 为奇数时保持因卷积导致的序列长度不变（same padding）。
输入 x 的形状被转换为 (batch, channels, seq_len) 以适配 Conv1d，最后返回 (batch, seq_len, output_size) 方便后续模块（Transformer）使用。
input_size 这里代表输入通道数（3），output_size 通常是后续 Transformer 的 d_model（例如 64
'''
class TCN(nn.Module):
...
# 整体情感识别模型
'''
TCN：提取时序低层特征（将原始 3 通道映射到 hidden_size）。
Transformer：进一步建模序列中长程依赖。
classifier：对全局池化后的 hidden_size 向量做线性分类
'''
...
# 有监督训练
'''
有监督训练函数，结构基本与 pretrain 相同，但没有任何 epoch 打印或日志，建议加上 model.train()、loss 累计和打印
'''
...
'''
对前 1000 个样本（限制以节约时间）应用变换生成自监督数据。
每个原始窗口产生3个变换样本（原始、噪声、扭曲），因此 pretrain_data.shape应为 (1000*3, window_size, 3)
（前提是 X 足够多）。
pretrain_labels 包含 0/1/2，表示变换类型
'''
...
'''
将 pretrain 数据做成 TensorDataset 并放入 DataLoader
'''
